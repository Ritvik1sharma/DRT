#ifndef SCHEDULER_8_H
#define SCHEDULER_8_H
...
class Scheduler_8{
	...
	public:
		...
		Scheduler_8(Matrix * mtx, Parameters * params, Stats * stats, LLB_Mem * llb);
	...
	private:
		...
		//#else
        	//Version for diagonal DRT
			void ExtractBTopTile(int k_idx_start, int j_idx_start, std::map<Dim, int> &end_top);
		//#endif
		...
		uint64_t multiplyOneARowInLogOutput(int i_idx, int j_start_top, int j_end_top, int k_start_top, int k_end_top);
