...
int main(int argc, char *argv[])
{
   ...
   std::string filename = ip->p_input1;
   char fname[100];
   ...
   // Keeps track of cycles, timing, traffic, and bandwidth
	...
	// LLB memory
	LLB_Mem * llb = new LLB_Mem(stats, params, ip->p_llb_size, ip->p_a_llb_perc, ip->p_b_llb_perc, ip->p_o_llb_perc);
  printf("After LLB_mem\n");
  fflush(stdout);
  
	Matrix * mat = new Matrix(ip->p_input1, ip->p_input2, params, stats, llb);
  printf("After new matrix\n");
  ...
  fflush(stdout);
	...
	sched_8->Run();
	std::cout << " Get run time " << std::endl;
...
void initializeInputParameters(input_parameters_st *input_params){
	...
	input_params->p_static_i = 4; input_params->p_static_j = 4; input_params->p_static_k = 4;
	...
	input_params->p_bottom_intersect= intersect::idealModel; input_params->p_middleDOT_traffic = middleDOTTrafficStatus::no;
